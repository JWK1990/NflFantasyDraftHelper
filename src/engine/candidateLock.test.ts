...
describe("candidate lock", () => {
  ...
  it("lets Taylor ADP change the availability badge without dropping Taylor from his evaluation", () => {
    ...
    expect(
      row!.reasons.some((reason) => reason.startsWith("Unlikely to be available at pick 19")),
    ).toBe(true);
  });
  ...
  it("targets availability badges at pick 19 during opponent turns and pick 30 on the user's turn", () => {
    ...
    const likely16 = chips(at16, "likely available at");
    ...
    const likely19 = chips(at19, "likely available at");
