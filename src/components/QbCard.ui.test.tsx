...
import type { WatchlistTip } from "../engine/watchlistTips.ts";
...
describe("draft tip bar", () => {
  ...
  it("puts the upcoming league-winner reminder on the collapsed bar", () => {
    render(<QbCardView card={qbCard} watchlistTips={[tip]} />);
    expect(screen.getByText("Upcoming watchlist players")).toBeTruthy();
  ...
  it("still shows a league-winner tip after the QB card drops away", () => {
    render(<QbCardView card={null} watchlistTips={[tip]} />);
    expect(screen.getByText("Upcoming watchlist players")).toBeTruthy();
    expect(screen.getAllByRole("listitem").map((item) => item.textContent)).toEqual([
      "Washington (ADP 94, Rank 8, Picks Before 14)",
    ]);
  ...
  it("lists several upcoming league winners as bullet points on the bar", () => {
    render(<QbCardView card={qbCard} watchlistTips={[tip, allenTip]} />);
    expect(screen.getByText("Upcoming watchlist players")).toBeTruthy();
